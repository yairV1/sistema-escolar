...
@section('content')
    <div class="container-fluid p-3 p-md-4">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
            <h1 class="h4 fw-semibold font-serif mb-0">Anotaciones</h1>
            @if ($estudiantes->isEmpty())
                <button type="button" class="btn btn-primary btn-sm" disabled title="Necesitás al menos un estudiante activo">
                    <i class="fas fa-plus me-1"></i> Nueva anotación
                </button>
            @else
                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalNuevaAnotacion">
                    <i class="fas fa-plus me-1"></i> Nueva anotación
                </button>
            @endif
        </div>
        ...
        <form method="GET" action="{{ route('docente.anotaciones.index') }}" class="row g-2 mb-3 align-items-center" data-autosubmit-form>
            ...
            <div class="col-md-2 d-grid">
                <a href="{{ route('docente.anotaciones.index') }}" class="btn btn-outline-secondary btn-sm">Limpiar filtros</a>
            </div>
        </form>
        ...
        @if ($anotaciones->isEmpty())
            <div class="empty-state">
                ...
                @if (($filtros['q'] ?? '') || ($filtros['tipo'] ?? '') || ($filtros['estado'] ?? ''))
                    <p class="mb-0">No hay anotaciones que coincidan con los filtros.</p>
                @else
                    <p class="mb-0">Todavía no registraste anotaciones sobre tus estudiantes.</p>
                @endif
            </div>
        @else
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <button type="button" class="btn btn-sm btn-outline-secondary" title="Editar"
                                            data-bs-toggle="modal" data-bs-target="#modalEditarAnotacion{{ $obs->id_observacion }}">
                                        <i class="fas fa-pen"></i>
                                    </button>
                                    @if ($obs->estado === 'activa')
                                        <button type="button" class="btn btn-sm btn-outline-danger" data-desactivar
                                                data-url="{{ route('docente.anotaciones.desactivar', $obs) }}"
                                                data-nombre="esta anotación"
                                                title="Archivar">
                                            <i class="fas fa-box-archive"></i>
                                        </button>
                                    @else
                                        <button type="button" class="btn btn-sm btn-outline-success" data-activar
                                                data-url="{{ route('docente.anotaciones.activar', $obs) }}"
                                                data-nombre="esta anotación"
                                                title="Reactivar">
            ...
            {{ $anotaciones->links('pagination::bootstrap-5') }}
        @endif
    </div>
...
@push('modals')
    @php
        ...
        // Opciones del combobox de estudiante (ver resources/js/components/combobox.js):
        // filtrado en el cliente, así que se manda la lista completa una sola vez acá
        // en vez de un <select> largo para scrollear.
        $estudiantesOptions = $estudiantes->map(fn ($estudiante) => [
            'value' => $estudiante->id_estudiante,
            'label' => trim($estudiante->usuario->nombres.' '.$estudiante->usuario->apellidos),
            'sub' => $estudiante->codigo_estudiante,
        ])->values();
    @endphp

    {{-- Modal: nueva anotación --}}
    <div class="modal fade" id="modalNuevaAnotacion" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form data-crud-form data-url="{{ route('docente.anotaciones.store') }}" novalidate>
                    <div class="modal-header">
                        <h5 class="modal-title font-serif">Nueva anotación</h5>
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <div class="combobox" data-combobox data-options="{{ $estudiantesOptions->toJson() }}">
                                    <input type="text" class="form-control combobox-input" placeholder="Escribí el nombre o código..." autocomplete="off">
                                    <input type="hidden" class="form-control combobox-hidden" name="id_estudiante" data-feedback="err-no-estudiante">
    ...
    {{-- Modales: editar anotación (uno por fila) --}}
    @foreach ($anotaciones as $obs)
        <div class="modal fade" id="modalEditarAnotacion{{ $obs->id_observacion }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form data-crud-form data-url="{{ route('docente.anotaciones.update', $obs) }}" novalidate>
                        <div class="modal-header">
                            <h5 class="modal-title font-serif">Editar anotación</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row g-3">
                                <div class="col-12">
                                    <label class="form-label">Estudiante *</label>
                                    <div class="combobox" data-combobox data-options="{{ $estudiantesOptions->toJson() }}"
                                         data-selected-value="{{ $obs->id_estudiante }}"
                                         data-selected-label="{{ trim($obs->estudiante->usuario->nombres.' '.$obs->estudiante->usuario->apellidos) }}">
                                        <input type="text" class="form-control combobox-input" placeholder="Escribí el nombre o código..." autocomplete="off">
                                        <input type="hidden" class="form-control combobox-hidden" name="id_estudiante" data-feedback="err-eo-estudiante-{{ $obs->id_observacion }}">
                                        <div class="combobox-list"></div>
                                    </div>
